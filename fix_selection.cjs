    ...
    if (drag?.type === "box-select" && e.button === 0) {
      ...
      if (maxX - minX > 0.1 && maxY - minY > 0.1) {
        ...
        this.doc.shapes.forEach(s => {
           if (s.kind === "rect" && inBox(s.pos[0], s.pos[1])) this.selection.push({ kind: "shape", id: s.id });
           else if (s.kind === "polygon" && s.points.some(p => inBox(p[0], p[1]))) this.selection.push({ kind: "shape", id: s.id });
        });
...
// 12. placeEntity and finishDraft
code = code.replace(
